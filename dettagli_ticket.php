<?php
  ...
  if(isset($_SESSION['user'])){
    ...
    //mi prendo l'id del ticket con get e poi eseguo la query per le informazioni
    if(isset($_GET['numero'])){
      $numero=$_GET['numero'];

      $q_ticket=" SELECT Ticket.tipologia, Ticket.stato, Ticket.data_ticket, Ticket.titolo, Ticket.testo
                  FROM Ticket Join Utente on Ticket.cod_utente = Utente.id
                  WHERE Ticket.id = '$numero'";
      
      $ret = pg_query($dbconn, $q_ticket);
      $row2 = pg_fetch_row($ret);

      $stato=$row2[1];
      $data= date_format(date_create($row2[2]), 'd-m-Y');
      $tipologia=$row2[0];
      $titolo=$row2[3];
      $testo=$row2[4];
        ...
        <div class="cont-info background-card">
          <div class='row spazio'> 
            <div class='col-md-4 to-left'> 
              <p class="title-ord">TICKET #<span><?php if(isset($numero)){ echo($numero);}?></span></p>
            </div>
              ...
              <form id="request-form" class="needs-validation">
                ...
                <div class="mb-4 pdlf-10px">
                  <h4 class="riep-title ticket">Tipologia: 
                    <span  class="riep-text ticket"> <?php if(isset($tipologia)){ echo $tipologia;} ?> </span>
                  </h4>
                </div>
                <div class="mb-4 pdlf-10px">
                  <div class="form-label-group margin-label">
                    <h4 class="riep-title ticket">Oggetto: 
                      <span  class="riep-text ticket"><?php if(isset($titolo)){ echo $titolo;} ?> </span>
                    </h4>
                  </div>
                </div>
                <div class="mb-12px pdlf-10px">
                  <div class="form-label-group">
                    <h4 class="riep-title ticket">Testo: </h4>
                    <span  class="riep-text ticket"><?php if(isset($testo)){ echo $testo;} ?></span>
                  </div>
                </div>
              </form>
            </div>
            <div class="col-riep brd">
              <h3 class="mb-3" style="text-align: center;">
                <span style="color: gold; text-transform:uppercase;">Le tue informazioni </span>
              </h3>
              <div class="riep">
